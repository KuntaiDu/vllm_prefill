...
details.
python bm_prom.py  --prefill-only --shuffle-seed 10 --output-len 10 --num-documents  8 --num-users 5 --user-history-length 2
...
def main(args):
    ...
    for user_idx in range(args.num_users):
        user_alpha = user_alphas[user_idx]  # 每个用户的随机后缀
        user_name = f"user{user_idx}"

        # 每个用户的 user_history 直接是 user_idx + alpha，重复 user_history_length * repeat_user 次
        ...
        for doc_chunk in documents:
            ...
            record = {
                "id": user_name,  # id直接用 user0、user1
                "conversations": [
